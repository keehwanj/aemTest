...
(function ($) {
  ...
  $.fn.tabs = function (initial, settings) {
    ...
    settings = $.extend(
      {
        initial:
          initial && typeof initial == "number" && initial > 0 ? --initial : 0,
        disabled: null,
    ...
    $.browser.msie6 =
      $.browser.msie &&
      (($.browser.version && $.browser.version < 7) ||
        /MSIE 6.0/.test(navigator.userAgent)); // do not check for 6.0 alone, userAgent in Windows Vista has "Windows NT 6.0"
    ...
    return this.each(function () {
      ...
      if (settings.remote) {
        tabs.each(function () {
          ...
          $(this).bind("loadRemoteTab", function (e, callback) {
            ...
            setTimeout(function () {
              // Timeout is again required in IE, "wait" for id being restored
      ...
      containers =
        ...
        $(">" + settings.tabStruct, container); // fallback to default structure

      // attach classes for styling if not present
      nav.is("." + settings.navClass) || nav.addClass(settings.navClass);
      containers.each(function () {
        var $$ = $(this);
        $$.is("." + settings.containerClass) ||
          $$.addClass(settings.containerClass);
      });

      // try to retrieve active tab from class in HTML
      var hasSelectedClass = $("li", nav).index(
        $("li." + settings.selectedClass, nav)[0]
      );
      ...
      containers
        .filter(":eq(" + settings.initial + ")")
        .show()
        .end()
        .not(":eq(" + settings.initial + ")")
        .addClass(settings.hideClass);
      $("li", nav)
        .removeClass(settings.selectedClass)
        .eq(settings.initial)
        .addClass(settings.selectedClass); // we need to remove classes eventually if hash takes precedence over class
      ...
      if (settings.fxAutoHeight) {
        ...
        var _setAutoHeight = function (reset) {
          ...
          if ($.browser.msie6) {
            containers.each(function () {
              ...
              this.style.setExpression(
                "behaviour",
                'this.style.height = this.minHeight ? this.minHeight : "1px"'
              ); // using an expression to not make print styles useless
            });
          } else {
        ...
        var watchFontSize =
          $("#tabs-watch-font-size").get(0) ||
          $('<span id="tabs-watch-font-size">M</span>')
            .css({
              display: "block",
              position: "absolute",
              visibility: "hidden",
        ...
        setInterval(function () {
          ...
          if (
            ...
            currentFontSize != cachedFontSize
          ) {
            _setAutoHeight(
              currentWidth > cachedWidth || currentFontSize < cachedFontSize
            ); // if heights gets smaller reset min-height
      ...
      if (settings.fxSlide || settings.fxFade) {
        ...
        if (settings.fxShow) {
          ...
          // use some kind of animation to prevent browser scrolling to the tab
      ...
      tabs.bind("triggerTab", function () {
        ...
        if (
          container.locked ||
          li.is("." + settings.selectedClass) ||
          li.is("." + settings.disabledClass)
        ) {
          return false;
        }
        ...
        if ($.browser.msie) {
          ...
          var tempForm = $(
            '<form action="' +
              hash +
              '"><div><input type="submit" value="h" /></div></form>'
          ).get(0); // no need to append it to the body
      ...
      tabs.bind("disableTab", function () {
        var li = $(this).parents("li:eq(0)");
        if ($.browser.safari) {
          /* fix opacity of tab after disabling in Safari... */
      ...
      tabs.bind("enableTab", function () {
        ...
        if ($.browser.safari) {
          /* fix disappearing tab after enabling in Safari... */
      ...
      tabs.bind("click", function (e) {
        ...
        if (
          ...
          (typeof onClick == "function" &&
            onClick(this, toShow[0], toHide[0]) === false)
        ) {
          this.blur();
          return false;
        }
        ...
        if (toShow.size()) {
          ...
          if (!$.browser.msie) {
            // not in IE to prevent ClearType font issue
            resetCSS["opacity"] = "";
          }
