...
//String brand = properties.get("brand", "Government of New Brunswick");
  	String path = properties.get("path", "/content/vnbw/en/search");
	String att_title = properties.get("att_title", "Voice of NB Women");
        ...
        //out.write("<nav class=\"navbar navbar-default\" role=\"navigation\">");        
        	out.write("<div class=\"navbar-header\">");
        		...
        		//out.write("<a class=\"navbar-brand visible-xs\" href=\"#\">Government of New Brunswick</a>");   
  				out.write(String.format("<a class=\"navbar-brand visible-xs\" href=\"#\">%s</a>", brand));        		
            ...
            if(child!=null && !child.isHideInNav()) {
                ...
                for (Iterator<Page> j = child.listChildren(); j.hasNext(); ) {
                    ...
                    if(grandchild.getPath().equals(currentPage.getPath())) cssClass+= " active";
                        grandchildren += String.format("<li class=\"parent\"><a href=\"%s.html\" class=\"btn_nav\"><b class=\"caret-right\"></b> %s</a></li>",grandchild.getPath(),title);
                    }
                }
                ...
                if(grandchildren.length()>0){
					//first parents do not have any content so that we do not need to have a link to the first parent page.                    
                    navItem = String.format("<li class=\"parent %s %s\"><a %s href=\"%s.html\" title=\"%s\" class=\"btn_nav %s\" data-toggle=\"dropdown\">%s <b class=\"caret\"></b></a><ul <ul class=\"dropdown-menu\">%s</ul></li>","dropdown", cssClass,multilineStyle ,child.getPath(),cleantitle, "dropdown-toggle" ,title,grandchildren);
                } 
                out.write(navItem);
            }
        }
