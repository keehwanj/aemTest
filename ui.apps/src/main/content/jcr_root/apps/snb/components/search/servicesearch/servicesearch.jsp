...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-12 col-md-8 col-lg-8">
        ...
        <div id="search_box">
            <form action="${currentPage.path}.html" class="form-horizontal" id="searchForm_"
                  onsubmit="return validate(this);" name="search_form" role="form">
                <input type="hidden" name="_charset_" value="utf-8">
                <input id="start" name="start" type="hidden" value="${start}"/>
                <input id="hits" name="hits" type="hidden" value="${hits}"/>
                <div class="panel panel-default">
                    <div class="panel-heading"><i class="glyphicon glyphicon-search"></i> <fmt:message
                            key="serviceSearchTitle"/></div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="keyword" class="form-control servicekeyword" id="keyword"
                               value="<%=encodedKeyword%>" placeholder="<fmt:message key="keyword"/>"
                               onclick="this.value='';">
                    </div>
                </div>
                <!--START of submit button-->
                <div class="form-group">
                    <button type="submit" class="btn btn-default btn-sm center-block searchButton"><span
                            class="glyphicon glyphicon-search"></span> <fmt:message key="contactSearchButtonText"/>
                    </button>
                </div>
                    ...
                    <c:when test="<%=!properties.get("hideFacets", false) && result.getFacets()!=null && result.getFacets().get("tagid")!=null && result.getFacets().get("tagid").getBuckets()!=null%>">
                        ...
                        <div id="search_services_left_nav">
                            ...
                            <ul>
                                ...
                                %>
                                <li><c:out value="<%=key%>"/> <span class="badge"><c:out
                                        value="<%=orderedBucket.get(key).get(0)%>"/></span> - <a
                                        href="<cq:requestURL><cq:removeParam name="tagid" value="<%=orderedBucket.get(key).get(1)%>"/><cq:removeParam name="facet" value="true"/></cq:requestURL>"><fmt:message
                                        key="filter"/></a></li>
                                <%
                                } else {
                                %>
                                <li><a title="<fmt:message key="filterResults"/>"
                                       href="<cq:requestURL><cq:addParam name="tagid" value="<%=orderedBucket.get(key).get(1)%>"/><cq:addParam name="facet" value="true"/></cq:requestURL>">
                                    <c:out value="<%=key%>"/> <span class="badge"><c:out
                                        value="<%=orderedBucket.get(key).get(0)%>"/></span>
                                </a></li>
                                <%
                                        }
                                    }
                        ...
                        <c:if test="<%= !properties.get("hideFacets", false) && tagids!=null && tagids.length>0 %>">
                            ...
                            <div id="search_services_left_nav">
                                ...
                                <ul>
                                    <c:forEach var="tagid" items="${tagids}">
                                        <c:set var="tag"
                                               value="<%= tm.resolve((String) pageContext.getAttribute("tagid")) %>"/>
                                        <li>${tag.title} (<%= result.getTotalMatches() %>) - <a
                                                href="<cq:requestURL><cq:removeParam name="tagid" value="${tagid}"/><cq:removeParam name="facet" value="true"/></cq:requestURL>"><fmt:message
                                                key="filter"/></a></li>
                                    </c:forEach>
                                </ul>
                            </div>
...
//https://helpx.adobe.com/au/experience-manager/kb/Unclosed-ResourceResolver-warnng-at-com-day-cq-search-impl-builder-QueryBuilderImpl.html
    ...
    if(Utils.isNotEmpty(keyword)){
        ...
        if (resources.hasNext()) {
            ...
            log.error("closing resourceResolver-snb service search");
        }
    }
%>