...
%>
<form action="<%=getLocalization("searchPath","//www2.gnb.ca/content/gnb/"+ lang + "/sitesearch",currentPage,log)%>.html" title="<%=getLocalization("searchHint","Search GNB.ca",currentPage,log) %>" method="get" id="keyword_search_form" onsubmit="if(this.q.value=='')return false;return true">
	<label for="searchText"><%=getLocalization("searchLabel","New Brunswick Site Search",currentPage,log)%></label>
    <input type="text"  name="q" placeholder="<%=inputValue%>" id="searchText" placeholder="Search" class="searchText ui-autocomplete-input" aria-labelledby="searchText"/>
	<%if(lang=="fr") { %>
