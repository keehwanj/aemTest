...
<div class="form-group">
  ...
  <div class="col-sm-7">  
  <select class="form-control <%=gtm_value.trim()%>" id="<%=title%>" name="<%=gtm_value.trim()%>" onChange="openWindow(this.value, this.options[this.selectedIndex].getAttribute('target'))">
            <option value="#"><%=optionText%></option>        
                <%for(int i=0; i<min; i++) {
                    if (url[i].startsWith("/")) {
                        if((url[i].endsWith(".pdf"))||(url[i].endsWith(".jpg"))||(url[i].endsWith(".gif"))||(url[i].endsWith(".PDF"))||(url[i].endsWith(".JPG"))||(url[i].endsWith(".GIF"))){%>
