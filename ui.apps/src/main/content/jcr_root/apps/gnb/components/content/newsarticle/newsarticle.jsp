...
<%
  ...
  try {   
    ...
    log.error("news article page unable to get data node");
  }
    ...
    try {
      ...
      log.error("news article data not tagged with a department");
    }
%>
...
<%
    ...
    if (Utils.isNotEmpty(props.get("mediaAssetRef", ""))) {
      ...
      String imageTag = String.format("<img id=\"switcherTarget\" class=\"img-responsive\" src=\"%1s\"/>", getRendition(resource,props.get("mediaAssetRef", "")));
      ...
      try {
        ...
        log.error("Exception occurred rendering multimedia for newsarticle");
      }
      %> 
...
<%
  ...
  log.error("Something bad happened on the newsarticle");
  %><cq:include script="empty.jsp" /><%
}
%>