...
/*END of XSSS*/
  ...
  if (configuationPage != null) {
      ...
      // line below - this gets us a resource that points to (is associated with ) /content/cq:tags/gnb-eng/organizations
     ...
     if ((Utils.isNotEmpty(encodedKeyword) && !encodedKeyword.equals(keywordLabel))&& !Utils.isNotEmpty(fname) && !Utils.isNotEmpty(lname) && !Utils.isNotEmpty(properties.get("departmentId", ""))) {
      ...
      if (Utils.isNotEmpty(lname) || Utils.isNotEmpty(fname) || Utils.isNotEmpty(properties.get("departmentId", "")) ) {
         ...
         if (Utils.isNotEmpty(encodedFname)) {
            map.put("1_property", "contactinfo/FIRSTNAME_STRIP");
            map.put("1_property.operation", "like");      
            map.put("1_property.value", toUpperCaseSansAccent(encodedFname.replace("'","''").toUpperCase()) + "%");                  
         }
         if (Utils.isNotEmpty(encodedLname)) {
            map.put("2_property", "contactinfo/LASTNAME_STRIP");
            map.put("2_property.value", toUpperCaseSansAccent(encodedLname.replace("'","''").toUpperCase()) + "%");
            map.put("2_property.operation", "like");
         }
         if (Utils.isNotEmpty(deptId)) {
            map.put("3_property", "contactinfo/DEPTID");
            map.put("3_property.value", deptId);
