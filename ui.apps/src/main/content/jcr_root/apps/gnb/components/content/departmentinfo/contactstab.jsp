...
try {
  ...
  //request from Paula, Connie(2022-04-25)  
  String ORGTYPE_text = "Fermetures des Centres de services de SNB";  
  String ORGTYPE_link = "https://www2.snb.ca/content/snb/fr/a-propos/bureaux.html";  

  if (lang == null || lang.equalsIgnoreCase("en")) {
    langSuffix = "ENG";
    //request from Paula, Connie(2022-04-25)  
    ORGTYPE_text = "SNB Service Centre Closures";    
    ORGTYPE_link = "https://www2.snb.ca/content/snb/en/about/office-closures.html";    
  }
...
<%
  ...
  if (p_orgDataNode.hasProperty(ORGURLINTERNET) && Utils.isNotEmpty(p_orgDataNode.getProperty(ORGURLINTERNET).getString())) {
    ...
    //request from Paula, Connie(2022-04-25)
  if (p_orgDataNode.hasProperty(ORGTYPE) && Utils.isNotEmpty(p_orgDataNode.getProperty(ORGTYPE).getString())&& p_orgDataNode.getProperty(ORGTYPE).getString().trim().equals("93")) {
    out.write("<br/>");
    out.write(String.format("<a href=\"%s\">%s</a>", ORGTYPE_link, ORGTYPE_text));
  }
...
/*START of media contacts*/
    ...
    String contactList = properties.get("contactList", "");
      if(!Utils.isNotEmpty(contactList)){
      	if(lang.equals("en")){
			contactList="https://www.gnb.ca/en/gov/contacts/communications-contacts.html";
        }
        else{
			contactList="https://www.gnb.ca/fr/gouv/contacts/ressources-communications.html";
        }
      }
      else{
...
/*END of media contacts*/
  ...
  //if(isDepartment) {
    ...
    if (p_orgDataNode != null && p_contactDataPage != null) {
      ...
      map.put("1_property", "contactinfo/DEPTID");
      map.put("1_property.value", p_orgDataNode.getProperty("DEPTID").getString());

      map.put("2_property", "contactinfo/PERSONTYPEID");
      map.put("2_property.value", "71"); //Hard coded value is bad: indicated a the media contact type
      map.put("path", p_contactDataPage);

      if (!isDepartment ) {  //if not a department - see if there are any media contacts in this branch/unit 
          map.put("3_property", "contactinfo/ORGID");        
          map.put("3_property.value", parent_org_id);
       }
...
<%
  ...
  log.error("DepartmentInfo component unable to render contacts tab.");
}
