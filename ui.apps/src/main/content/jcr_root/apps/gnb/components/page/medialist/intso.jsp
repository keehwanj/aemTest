...
String SQL="";
String st="Television Stations - Outside Province";
...
while (rs.next()) 
    ...
    //creating node
    final Node gnbNode = WCMUtils.getNode(resourceResolver.getResource("/content/gnb/cnb_data/medialist/tso"));    
    nodeIter = gnbNode.getNodes();
    out.print(nodeIter.getSize());
    out.print("<br/>");    


    gnbNode.addNode("tso"+id, Nodes.PAGE.nodeType());//change 11 with id
    gnbNode.save();

    //setting jcr:content node
    final Node jcrContentNode = gnbNode.getNode("tso"+id);//change the "11" with id   
