...
<%
  ...
  String langSuffix_opposite="";
  String langSuffix2 = "FRE";
  Locale locale = currentPage.getLanguage(true);
  if (locale == null || locale.getLanguage().equalsIgnoreCase("en")) {
    lang = "en";
    langSuffix = "";
    langSuffix_opposite="FRE";  
    langSuffix2 = "ENG";
  }
%>
...
<%
  ...
  try {
    ...
    if (serviceDataNode != null) {
      ...
      //new field for sorting - 2021-02-26  
  ...
  <% 
    ...
    if (serviceDataNode.hasNode(SERVICEFEES) && serviceDataNode.getNode(SERVICEFEES).hasNodes()) {
        ...
        try {
          ...
          out.write(String.format("<th scope=\"col\" nowrap class=\"feeDate\">%s</th>", LocaleSupport.getLocalizedMessage(pageContext, "feesDate_new")));
          out.write("</tr>");
    ...
    <%
      ...
      try {
        for (String path : sortedContacts.values()) {
          %><sling:include path="<%=path + ".searchresult_embeded_narrow.html" %>" /><%
        }
        ...
        if (txnsNode.hasNodes()) {
          ...
          for (NodeIterator nodeIter = txnsNode.getNodes(); nodeIter.hasNext();) {
            ...
            if (childNode.hasProperty(SERVICESONLINEPAYMENT) && Utils.isNotEmpty(childNode.getProperty(SERVICESONLINEPAYMENT).getString())) {
                //key = childNode.getProperty(SERVICESONLINEPAYMENT).getString();//commented out(2021-02-26)
                //new for manual sorting(2021-02-26)
                if (childNode.hasProperty(displayorder) && Utils.isNotEmpty(childNode.getProperty(displayorder).getString())) {
                    key = childNode.getProperty(displayorder).getString()+childNode.getProperty(SERVICESONLINEPAYMENT).getString();
                }
                else{
        ...
        if (linksNode.hasNodes()) {
          ...
          for (Node childNode : sorted_map.values()) {
            if (childNode.hasProperty(SERVICESLINKSURL) && childNode.hasProperty(SERVICESLINKSDESCRIPTION) && Utils.isNotEmpty(childNode.getProperty(SERVICESLINKSURL).getString())
                ...
                String link_pdf=childNode.getProperty(SERVICESLINKSURL).getString().replace("showpdf","pdf");
				...
				String navItem = String.format("<li><a href=\"%1s\" title=\"%2s\">%3s</a></li>",link_pdf,childNode.getProperty(SERVICESLINKSDESCRIPTION).getString(),childNode.getProperty(SERVICESLINKSDESCRIPTION).getString());                    
              out.write(navItem);
            } else if (childNode.hasProperty(OTHERSERVICEID) && Utils.isNotEmpty(childNode.getProperty(OTHERSERVICEID).getString())) {
              Resource r = slingRequest.getResourceResolver().getResource(serviceDataPage + "/" + childNode.getProperty(OTHERSERVICEID).getString());


              //out.write(serviceDataPage + "/" + childNode.getProperty(OTHERSERVICEID).getString());
              if (r != null) {
        ...
        if (actsNode.hasNodes()) {
          ...
          for (NodeIterator nodeIter = actsNode.getNodes(); nodeIter.hasNext();) {
            ...
            if ((childNode.hasProperty(SERVICEACT)) && (childNode.hasProperty(SERVICEACTPDF)) && (childNode.hasProperty(SERVICEACTURL))) {
              ...
              //String navItem = String.format("%s&nbsp;<a href=\"%s\" title=\"%s\">HTML</a></li>", name, html, name);  
              String navItem = String.format("<a href=\"%s\" title=\"%s\">%s</a></li>", html,name, name);  
              out.write(navItem);
              ...
              for (NodeIterator subnodeIter = childNode.getNodes(); subnodeIter.hasNext();) {
                ...
                if ((grandchildNode.hasProperty(REGULATIONS)) && (grandchildNode.hasProperty(REGULATIONSNUM)) &&(grandchildNode.hasProperty(REGULATIONSURL))) {
                  ...
                  //navItem = String.format("<li>%s&nbsp;%s&nbsp;<a href=\"%s\" title=\"%s\">HTML</a></li>", number, name, html, name);  
                  navItem = String.format("<li><a href=\"%s\" title=\"%s\">%s&nbsp;%s</a></li>", html,name,number, name);  
                  out.write(navItem);
...
<script>
    $(document).ready(function() {
        $('a').each(function() {
            ...
            if (href) {
                // 정규 표현식을 사용하여 '_jcr_content/mainContent_par/services_renderer' 부분을 제거
                var updatedHref = href.replace(/\/_jcr_content\/mainContent_par\/services_renderer/g, '');
