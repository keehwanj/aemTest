...
<%!protected void writeOrgStructureNode(Node orgNode, int topDepth, StringBuilder sb, Page currentPage, Resource resource, Logger log) {
    //writeOrgStructureNode for listing organization 
    try {
  ...
  protected void writeGoogleDataNode(Node orgNode, Node secondaryOrgNode, int linkTarget, String parentalOverride, JspWriter out, Page currentPage, Resource resource, Logger log, String debugger) {
    try {
      ...
      if (orgNode != null && orgNode.hasNode("orginfo")) {
        ...
        if (secondaryOrgNode == null) {
          ...
          out.write("<table><tr><td style=\"font-size:100%;border-right:1px solid #D3D3D3;border-bottom:0px; width:50%\">");
          out.write(generateGoogleDataNodeContent(orgNode, currentPage, resource, log));
        ...
        //added for secondaryOrgNode expand, 
        //could not use secondaryOrgNode without "if (secondaryOrgNode != null)" that's why code became long

        if (linkTarget != 0) {
          String linkId = orgNode.getName();
          String linkId_s="";  
          if (secondaryOrgNode != null) {
  			linkId_s = secondaryOrgNode.getName();
          }

          if (linkTarget < 0) {
            Node uplink = orgNode;
            Node uplink_s = secondaryOrgNode;  
            for (int i = 0; i > linkTarget; i--){
  			  uplink = uplink.getParent();
              if(secondaryOrgNode!=null){
				uplink_s = uplink_s.getParent();  
              }
            }
            linkId = uplink.getName();
            if(secondaryOrgNode!=null){
				linkId_s = uplink_s.getName();  
            }
          }
          ...
          if (linkTarget > 0) {
            ...
            if(secondaryOrgNode!=null){
				out.write(String.format("<a class=\"chartExpander\" id=\"chartExpander%s\">%s</a>", linkId_s, Expand));   
            }
          } else {
            out.write(String.format("<a class=\"chartExpander\" id=\"chartExpander%s\">%s</a>", linkId, Contract));
            if(secondaryOrgNode!=null){
				out.write(String.format("<a class=\"chartExpander\" id=\"chartExpander%s\">%s</a>", linkId_s, Contract));   
            }  
          }
        }
  ...
  protected String generateGoogleDataTable(Node orgNode, String chartArray, int level, JspWriter out, Page currentPage, Resource resource, Logger log) {
    try {
      ...
      String parentalOverride = null;
        ...
        //orgNode is just one of department top node like /content/data/organizations/140 for social development, ORGLEVEL-01 property is minister
      if (orgNode.hasNode("orginfo") && orgNode.getNode("orginfo").hasProperty(ORGLEVEL) && orgNode.getNode("orginfo").getProperty(ORGLEVEL).getString().equalsIgnoreCase("01")) {
        ...
        //orgNode.getNodes()=nodeIter are nodes under department node like 1806,1787,6464,6483,15703,201632 under 140
        for (NodeIterator nodeIter = orgNode.getNodes(); nodeIter.hasNext();) {

          //nodeIter=orgNode.getNodes()-top department node's child nodes, (Node) nodeIter.next();-nodelter's siblings 
          Node child = (Node) nodeIter.next();

          //this if statement is for skipping orginfo node just under department node(140)  
          if (child.getName().equalsIgnoreCase("orginfo"))
            continue;

          //this if statement is for skipping jcr:content node just under department node(140)  
          if (child.getName().equalsIgnoreCase("jcr:content"))
            continue;

          //child.getNode("orginfo"); getting orginfo node under top department node like under /data/organization/140/1808
          ...
          if (orgInfoNode.getProperty(ORGTYPE).getString().equals("291") && orgInfoNode.getProperty(ORGLEVEL).getString().equals("01.01")) {
            minister = child;
            continue;
          }
          ...
          //treat a speaker of the house the same as a minister's office 
        ...
        if (minister != null) {
          ...
          if (minister.getNodes().getSize() > 2) {
            minLevel = 1;
            specialOrgTypeCase(minister, "ministerdata", chartArray, level, out, log, currentPage, parentalOverride, resource);
          }
          out.write("data.addRow(");
          //writeGoogleDataNode(minister, null, minLevel, parentalOverride, out, currentPage, resource, log, "");
          writeGoogleDataNode(minister, minister2, minLevel, parentalOverride, out, currentPage, resource, log, "");            
          out.write(");\n");
        ...
        if (deputyMinister != null) {
          ...
          if (deputyMinister.getNodes().getSize() > 2) {
            deptLevel = 1;
            specialOrgTypeCase(deputyMinister, "deputyministerdata", chartArray, level, out, log, currentPage, parentalOverride, resource);
          }
          out.write("data.addRow(");
          writeGoogleDataNode(deputyMinister, deputyMinister2, deptLevel, parentalOverride, out, currentPage, resource, log, "");
          out.write(");\n");
     ...
     for (NodeIterator nodeIter = orgNode.getNodes(); nodeIter.hasNext();) {
         ...
         if (childNode.getName().equalsIgnoreCase("orginfo"))
          ...
          //Premier Office- /content/data/organizations/168/920/orginfo (in order to display expand under Chief of Staff)
  ...
  private void specialOrgTypeCase(Node orgNode, String orgDataName, String chartArray, int level, JspWriter out, Logger log, Page currentPage, String parentalOverride, Resource resource) throws IOException, RepositoryException {
