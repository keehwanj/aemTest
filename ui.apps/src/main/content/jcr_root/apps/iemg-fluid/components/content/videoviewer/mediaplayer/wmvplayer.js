...
jeroenwijering.Player.prototype = {
  ...
  onLoadHandler: function (pid, tgt, sdr) {
    ...
    tgt.model = new jeroenwijering.Model(
      tgt.configuration,
      tgt.controller,
      tgt.view
    );
...
jeroenwijering.Controller.prototype = {
  ...
  setLoad: function (fil) {
    ...
    if (this.configuration["autostart"] == "true") {
      setTimeout(
        jeroenwijering.utils.delegate(this.model, this.model.goStart),
        100
      );
    }
  },
...
jeroenwijering.View.prototype = {
  ...
  onFullscreen: function (fss) {
    ...
    if (fst) {
      this.fstimeout = setTimeout(
        jeroenwijering.utils.delegate(this, this.hideFSControls),
        2000
      );
  ...
  onTime: function (elp, dur) {
    ...
    if (dur > 0) {
      ...
      snd.findName("RemainingText").Text = jeroenwijering.utils.timestring(
        dur - elp
      );
  ...
  resizeDisplay: function (wid, hei) {
    ...
    this.display.findName("OverlayCanvas")["Canvas.Left"] =
      wid - this.display.findName("OverlayCanvas").Width - 10;
...
jeroenwijering.Model = function (cfg, ctr, vie) {
  ...
  this.video.BufferingTime = jeroenwijering.utils.spanstring(
    this.configuration["bufferlength"]
  );
    ...
    if (stt != this.state) {
      ...
      if (stt != "Playing" && stt != "Buffering" && stt != "Opening") {
        ...
        this.timeint = setInterval(
          jeroenwijering.utils.delegate(this, this.timeChanged),
          100
        );
      }
    }
