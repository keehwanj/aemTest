...
jQuery(window).on('load', function () {
  ...
  jQuery('a').each(function () {
    ...
    if (badlinkJs.test(url)) {
      ...
      jQuery(this).attr('onclick', "ga('send', 'event', 'download', 'jpg', '" + url + "']);");
    } else if (isDownloadAsx.test(url)) {
      jQuery(this).attr('onclick', "ga('send', 'event', 'download', 'asx', '" + url + "']);");
    } else if (isDownloadDoc.test(url)) {
      jQuery(this).attr('onclick', "ga('send', 'event', 'download', 'doc', '" + url + "']);");
    } else {
    }
  }); //End each function
