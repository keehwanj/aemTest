/**START of jquery-ui-1.7.2.custom.min.js**/
/*
 * jQuery UI 1.7.2
 *
  ...
  (function (c) {
      ...
      focusable: function (k) {
        ...
        return (
          (/input|select|textarea|button|object/.test(l)
            ...
            : 'a' == l || 'area' == l
            ? k.href || !isNaN(j)
            : !isNaN(j)) && !c(k)['area' == l ? 'parents' : 'closest'](':hidden').length
        );
      },
      tabbable: function (k) {
    ...
    c.ui.mouse = {
      ...
      _mouseDistanceMet: function (j) {
        return (
          Math.max(Math.abs(this._mouseDownEvent.pageX - j.pageX), Math.abs(this._mouseDownEvent.pageY - j.pageY)) >=
...
(function (c) {
  ...
  c.extend(c.ui.dialog.overlay, {
    ...
    maxZ: 0,
    events: c
      .map('focus,mousedown,mouseup,keydown,keypress,click'.split(','), function (d) {
...
(function (a) {
  a.widget('ui.tabs', {
    ...
    _tabId: function (b) {
      return (
        (b.title && b.title.replace(/\s/g, '_').replace(/[^A-Za-z0-9\-_:\.]/g, '')) || this.options.idPrefix + a.data(b)
      );
    },
    ...
    _tabify: function (n) {
      ...
      for (var g = 0, m; (m = this.lis[g]); g++) {
        a(m)[a.inArray(g, d.disabled) != -1 && !a(m).hasClass('ui-tabs-selected') ? 'addClass' : 'removeClass'](
          'ui-state-disabled'
        );
      }
      if (d.cache === false) {
    ...
    _hideDatepicker: function (input, duration) {
      ...
      if (this._datepickerShowing) {
        ...
        if (duration != '' && $.effects && $.effects[showAnim]) {
          ...
          inst.dpDiv[
            duration == '' ? 'hide' : showAnim == 'slideDown' ? 'slideUp' : showAnim == 'fadeIn' ? 'fadeOut' : 'hide'
    ...
    _generateHTML: function (inst) {
      ...
      if (maxDate) {
        var maxDraw = this._daylightSavingAdjust(
          new Date(maxDate.getFullYear(), maxDate.getMonth() - numMonths[1] + 1, maxDate.getDate())
        );
        ...
        for (var col = 0; col < numMonths[1]; col++) {
          ...
          for (var dRow = 0; dRow < numRows; dRow++) {
            ...
            for (var dow = 0; dow < 7; dow++) {
              ...
              var unselectable =
                otherMonth || !daySettings[0] || (minDate && printDate < minDate) || (maxDate && printDate > maxDate);
              tbody +=
                '<td class="' +
...
jQuery.effects ||
  (function (d) {
    ...
    function c(g, f) {
      ...
      var j =
        i.callback || (d.isFunction(g[1]) && g[1]) || (d.isFunction(g[2]) && g[2]) || (d.isFunction(g[3]) && g[3]);
      return [g[0], i, h, j];
    }
    d.fn.extend({
      _show: d.fn.show,
    ...
    function b(g) {
      ...
      if (
        (f = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/.exec(g))
      ) {
...
/**END of jquery-ui-1.7.2.custom.min.js**/
