...
jeroenwijering.View = function (cfg, ctr) {
  ...
  this.configuration['sender'].getHost().content.onResize = jeroenwijering.utils.delegate(this, this.resizePlayer);
  this.configuration['sender'].getHost().content.onFullScreenChange = jeroenwijering.utils.delegate(
    this,
    this.onFullscreen
  );
...
jeroenwijering.View.prototype = {
  ...
  onFullscreen: function (fss) {
    ...
    if (fst) {
      this.fstimeout = setTimeout(jeroenwijering.utils.delegate(this, this.hideFSControls), 2000);
      this.fslistener = this.display.addEventListener(
        'MouseMove',
  ...
  onState: function (old, stt) {
    ...
    try {
      if (!(old == 'Completed' && stt == 'Buffering') && !(old == 'Buffering' && stt == 'Paused')) {
  ...
  assignColorsClicks: function () {
    ...
    if (this.configuration['linkfromdisplay'] == 'false') {
      this.display.addEventListener(
        'MouseLeftButtonUp',
        jeroenwijering.utils.delegate(this.controller, this.controller.setPlay)
      );
    } else {
      this.display.addEventListener(
        'MouseLeftButtonUp',
        jeroenwijering.utils.delegate(this.controller, this.controller.setLink)
      );
      this.display.findName('PlayIcon').Visibility = 'Collapsed';
    }
    if (this.configuration['logo'] != '') {
      this.display.findName('OverlayCanvas').Visibility = 'Visible';
      this.display.findName('OverlayLogo').ImageSource = this.configuration['logo'];
    }
    this.controlbar.findName('ControlbarBack').Fill = '#FF' + this.configuration['backcolor'];
    this.assignButton('Play', this.controller.setPlay);
    this.assignButton('Stop', this.controller.setStop);
    this.configuration['sender'].findName('ElapsedText').Foreground = '#FF' + this.configuration['frontcolor'];
    this.assignSlider('Time', this.changeTime);
    this.configuration['sender'].findName('DownloadProgress').Fill = '#FF' + this.configuration['frontcolor'];
    this.configuration['sender'].findName('RemainingText').Foreground = '#FF' + this.configuration['frontcolor'];
  ...
  assignButton: function (btn, act) {
    ...
    el1.Cursor = 'Hand';
    el1.addEventListener('MouseLeftButtonUp', jeroenwijering.utils.delegate(this.controller, act));
    el1.addEventListener('MouseEnter', jeroenwijering.utils.delegate(this, this.rollOver));
    el1.addEventListener('MouseLeave', jeroenwijering.utils.delegate(this, this.rollOut));
    this.configuration['sender'].findName(btn + 'Symbol').Fill = '#FF' + this.configuration['frontcolor'];
    try {
      this.configuration['sender'].findName(btn + 'OffSymbol').Fill = '#FF' + this.configuration['frontcolor'];
    } catch (e) {}
  },
  ...
  assignSlider: function (sld, act) {
    ...
    el1.Cursor = 'Hand';
    el1.addEventListener('MouseLeftButtonUp', jeroenwijering.utils.delegate(this, act));
    el1.addEventListener('MouseEnter', jeroenwijering.utils.delegate(this, this.rollOver));
    el1.addEventListener('MouseLeave', jeroenwijering.utils.delegate(this, this.rollOut));
    this.configuration['sender'].findName(sld + 'Slider').Fill = '#FF' + this.configuration['frontcolor'];
    this.configuration['sender'].findName(sld + 'Highlight').Fill = '#FF' + this.configuration['frontcolor'];
    this.configuration['sender'].findName(sld + 'Symbol').Fill = '#FF' + this.configuration['frontcolor'];
  },
  ...
  rollOver: function (sdr) {
    ...
    this.configuration['sender'].findName(str + 'Symbol').Fill = '#FF' + this.configuration['lightcolor'];
    try {
      this.configuration['sender'].findName(str + 'OffSymbol').Fill = '#FF' + this.configuration['lightcolor'];
    } catch (e) {}
  },

  rollOut: function (sdr) {
    var str = sdr.Name.substr(0, sdr.Name.length - 6);
    this.configuration['sender'].findName(str + 'Symbol').Fill = '#FF' + this.configuration['frontcolor'];
    try {
      this.configuration['sender'].findName(str + 'OffSymbol').Fill = '#FF' + this.configuration['frontcolor'];
    } catch (e) {}
  },
  ...
  resizeDisplay: function (wid, hei) {
    ...
    this.display.findName('OverlayCanvas')['Canvas.Left'] = wid - this.display.findName('OverlayCanvas').Width - 10;
  ...
  resizeControlbar: function (wid, yps, alp) {
    ...
    if (this.configuration['showdigits'] == 'true' && wid - rgt - lft > 160) {
      ...
      this.controlbar.findName('RemainingButton').Visibility = 'Collapsed';
...
jeroenwijering.Model = function (cfg, ctr, vie) {
  ...
  this.video.BufferingTime = jeroenwijering.utils.spanstring(this.configuration['bufferlength']);
  this.video.AutoPlay = true;
  this.video.AddEventListener('CurrentStateChanged', jeroenwijering.utils.delegate(this, this.stateChanged));
  this.video.AddEventListener('MediaEnded', jeroenwijering.utils.delegate(this, this.mediaEnded));
  this.video.AddEventListener('BufferingProgressChanged', jeroenwijering.utils.delegate(this, this.bufferChanged));
  this.video.AddEventListener('DownloadProgressChanged', jeroenwijering.utils.delegate(this, this.downloadChanged));
  ...
  stateChanged: function () {
    ...
    if (stt != this.state) {
      ...
      if (stt != 'Playing' && stt != 'Buffering' && stt != 'Opening') {
        ...
        this.timeint = setInterval(jeroenwijering.utils.delegate(this, this.timeChanged), 100);
      }
    }
  },
