...
public class Utils {
  ...
  public static String getDepartmentPath(String deptId, Resource resource) {
    ...
    String path = (String)(cache.get(deptId).getObjectValue());
    //manager.shutdown();
  ...
  public static String getQuicklink(String path, Page currentPage, Resource resource) {
    if (currentPage == null || resource == null || !isNotEmpty(path))
      return "";
      ...
      if (configPage != null) {
        ...
        for (String newsType : newsTypes) {
          ...
          if (isNotEmpty(dataPage) && thisPage.getPath().startsWith(dataPage.toString())) {
            ...
            // build the link text
            if (currentNode.hasProperty(String.format("%sTitle", lang))
            ...
            if (currentNode.hasProperty(String.format("%sDescription", lang))
        ...
        if (isNotEmpty(dataPage) && path.startsWith(dataPage.toString())) {
          ...
          if (assetNode != null && assetNode.hasNode(Nodes.JCR_CONTENT.nodeType())
        ...
        if (isNotEmpty(dataPage) && thisPage.getPath().startsWith(dataPage.toString())) {
          ...
          if (currentNode.getParent().hasNode("contactinfo")) {
            ...
            if (contactDataNode.hasProperty("LASTNAME")
                && isNotEmpty(contactDataNode.getProperty("LASTNAME").getString()))
        ...
        if (isNotEmpty(dataPage) && thisPage.getPath().startsWith(dataPage.toString())) {
          ...
          if (currentNode.getParent().hasNode("orginfo")) {
            ...
            // build the link text
            if (orgDataNode.hasProperty(String.format("ORGNAME%s", langSuffix))
            ...
            if (orgDataNode.hasProperty("INCHARGEGENDER") && isNotEmpty(orgDataNode.getProperty("INCHARGEGENDER").getString())  ) {
            ...
            if (!isDept && orgDataNode.hasProperty(ORGTITLE)
                && isNotEmpty(orgDataNode.getProperty(ORGTITLE).getString() ) ) {
        ...
        if (isNotEmpty(dataPage) && thisPage.getPath().startsWith(dataPage.toString())) {
          ...
          if (currentNode.getParent().hasNode("serviceinfo")) {
            ...
            // build the link text
            if (serviceDataNode.hasProperty(String.format("SERVICENAME%s", langSuffix2))
        ...
        if (isNotEmpty(selector) && isNotEmpty(rendererPage)) {
          String genericRendererBasePage = getConfigurationProperty("genericRendererBasePage_" + lang, configPage)
              + "/";
          ...
          if (currentPage.getPath().startsWith(departmentsPage)) {
            renderer = pageManager.getPage(rendererPage.replaceAll("<base>", departmentsPage).replace("<dept>", dept)
                .replace("<dept_alt>", ""));
            if (renderer == null) {
              renderer = pageManager.getPage(rendererPage.replaceAll("<base>", departmentsPage).replace("<dept>", "")
          ...
          if (renderer == null) {
            renderer = pageManager.getPage(rendererPage.replaceAll("<base>", genericRendererBasePage)
                .replace("<dept>", dept).replace("<dept_alt>", ""));
          }
          if (renderer == null) {
            renderer = pageManager.getPage(rendererPage.replaceAll("<base>", genericRendererBasePage)
                .replace("<dept>", "").replace("<dept_alt>", dept));
          }
          if (renderer == null) {
            renderer = pageManager.getPage(rendererPage.replaceAll("<base>", genericRendererBasePage)
                .replace("<dept>", "").replace("<dept_alt>", ""));
          }
          if (renderer != null) {
