...
public final class ServiceReplication {
  ...
  final private static String SERVICEKEYWORDS = "SERVICEKEYWORDS";
  final private static String SERVICEKEYWORDSFRE = "SERVICEKEYWORDSFRE";
  final private static String SERVICENAME = "SERVICENAME";
  final private static String SERVICENAMEFRE = "SERVICENAMEFRE";
  final private static String SERVICENAME_STRIP = "SERVICENAME_STRIP";
  final private static String SERVICENAMEFRE_STRIP = "SERVICENAMEFRE_STRIP";
  final private static String SERVICEFEES = "fees";
  final private static String SERVICEFEESHISTORY = "fees_history";
  final private static String SERVICEFEESREVENUE = "fees_revenue";
  final private static String SERVICEFORMS = "forms";
  final private static String SERVICELINKS = "related_links";
  final private static String ONLINETRANSACTIONS = "online_transactions";
  final private static String SERVICECONTACTS = "contacts";
  final private static String SERVICEACTS = "acts";

  private final static String SERVICE_SQL = "select st.* from servicestable st where st.servicetypeid in (2,3) and st.statusid = 'A' and st.internalservice = 'P' ";
  private final static String FEES_SQL = "select f.* from fees f inner join servicestable st ON f.servicesid = st.servicesid and st.servicetypeid in (2,3) "
      + "and st.statusid = 'A' and st.internalservice = 'P' order by f.servicesid, f.feelevel";
  private final static String FEE_HISTORY_SQL = "select fh.* from feeshistorytable fh inner join servicestable st ON fh.servicesid = st.servicesid and "
      + "st.servicetypeid in (2,3) and st.statusid = 'A' and st.internalservice = 'P' order by fh.servicesid, fh.feelevel";
  private final static String FEE_REVENUE_SQL = "select re.* from RevenueEstimatesTable re inner join servicestable st ON re.servicesid = st.servicesid and st.servicetypeid in (2,3) and "
      + "st.statusid = 'A' and st.internalservice = 'P' order by re.servicesid";
  private final static String FORM_SQL = "select sf.* from servicesforms sf "
  ...
  private final static String LINKS_SQL = "select sl.* from serviceslinks sl "
  ...
  private final static String TRANSACTIONS_SQL = "select op.* from onlinepaymenttable op "
  ...
  private final static String CONTACTS_SQL = "select sc.*, cg.GroupNameEng, cg.GroupNameFre, cg.DeptID from SERVICESCONTACTS sc "
  ...
  private final static String ACTS_SQL = "select sr.SERVICESID, a.* from ACTS a inner join SERVICEREGULATIONS sr on a.SERVICEACTID = sr.ACTSID "
  ...
  private final static String REGULATIONS_SQL = "select sr.SERVICESID, r.* from SERVICEREGULATIONS sr "
  ...
  private final static String MUNICIPALITY_SQL = "select m.* from municipalityForms m "; 
  ...
  private void createServices(Connection conn, Node serviceDataNode) throws SQLException, RepositoryException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        serviceNode.getNode(Nodes.JCR_CONTENT.nodeName()).setProperty("sling:resourceType",
            "gnb/components/page/gnbpage");
  ...
  private void createOnlineTransactions(Connection conn, Node serviceDataNode) throws SQLException, RepositoryException {
  ...
  private void createContacts(Connection conn, Node serviceDataNode) throws SQLException, RepositoryException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        // add new node and properties
        Node n = parentNode.addNode(rs.getString("ORGID") + "-" + rs.getString("GROUPID"), Nodes.UNSTRUCTURED
            .nodeType());
        for (int i = 0; i < columnNames.size(); i++) {
          n.setProperty(columnNames.get(i), rs.getString(i + 1));
        }
  ...
  private void createMunicipalityForms(Connection conn, Node municDataNode) throws SQLException, RepositoryException {
    ...
    try {
	  ...
	  log.error("Creating municipality forms node for " + municNodeName );
        ...
        municNode.getNode(Nodes.JCR_CONTENT.nodeName()).setProperty("sling:resourceType",
            "gnb/components/page/gnbpage");
