...
<form action="<%=getLocalization("searchPath","/content/enb/"+ lang + "/search",currentPage,log)%>.html" title="<%=getLocalization("searchHint","Search GNB.ca",currentPage,log) %>" method="get" id="keyword_search_form" onsubmit="if(this.q.value=='')return false;return true">
...
<input type="text" onclick="this.value='';" name="q" value="<%=inputValue%>" class="searchText" id="q">
      <%if(lang=="fr") { %>
        <input type="hidden" name="cx" value="004686105439310941724:7zgb7tm5n-u" />
      <%}else{ %>
        <input type="hidden" name="cx" value="004686105439310941724:wq2cq5n9ois" />
      <%} %>
