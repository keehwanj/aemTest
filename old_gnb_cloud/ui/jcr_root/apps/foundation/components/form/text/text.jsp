...
%><%@ page import="com.day.cq.wcm.foundation.TextFormat,
    ...
    %><div class="form_row form-group row">
        ...
        <div class="form_rightcol col-sm-8" id="<%= xssAPI.encodeForHTMLAttr(name) %>_rightcol"><%
            int i = 0;
            for (String value : values) {
                ...
                if (readOnly) {
                    ...
                    if (rows == 1) {
                        %><input class="<%= FormsHelper.getCss(properties, "form_field form_field_text form-control input-sm" + (multiValued ? " form_field_multivalued" : "" )) %>" <%
                            %>id="<%= xssAPI.encodeForHTMLAttr(currentId) %>" <%
                            %>name="<%= xssAPI.encodeForHTMLAttr(name) %>" <%
                            %>placeholder="<%= xssAPI.encodeForHTMLAttr(value) %>" value="<%= xssAPI.encodeForHTMLAttr(value) %>" onclick="this.value='';"<%   
                            %>size="<%= cols %>" <%
                            if (width != null) {
                        ...
                        %><textarea class="<%= FormsHelper.getCss(properties, "form_field form_field_textarea form-control input-sm") %>" <%
                            %>id="<%= xssAPI.encodeForHTMLAttr(currentId) %>" <%
                            %>name="<%= xssAPI.encodeForHTMLAttr(name) %>" <%
                            %>rows="<%= rows %>" cols="<%= cols %>" <%
                            if (width != null) {
...
$(document).ready(function(){
	$('.form_leftcolmark').each(function(){
    	var len=$(this).text().length
        if(len<=1){
            $(this).css("display","none")
            //$('.form_leftcolmark').text('test');
        }
	});
});
</script>
...
<script>        
$(document).ready(function(){
    //var text=$('.form_leftcolmark').text('');
})
</script>