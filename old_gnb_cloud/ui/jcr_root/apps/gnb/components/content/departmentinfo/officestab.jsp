...
try {
  ...
  if (p_deptDataNode != null && p_orgDataPage != null) {
    ...
    if (Utils.isNotEmpty(org_location)) {
        ...
        if(org_location_sa){
      		querymap.put("2_prop.value", "%Saint-Andr%");
            querymap.put("2_prop.operation", "like");
        }
 		//exception for Lamèque
        else if(org_location_la){
      		querymap.put("2_prop.value", "%Lam%");
            querymap.put("2_prop.operation", "like");
        }
 		//exception for Rivière-Verte
        else if(org_location_rv){
      		querymap.put("2_prop.value", "%Rivi%");
            querymap.put("2_prop.operation", "like");
        }
 		//exception for Saint-Léonard
        else if(org_location_sl){
      		querymap.put("2_prop.value", "%Saint-L%");
            querymap.put("2_prop.operation", "like");
        }
      ...
      //START checking/printing querybuilder
      /**
      out.write("<br/>");
      ...
      **/
	  //END checking/printing querybuilder
