  ...
  <%
    ...
    if (metadata.hasProperty("dam:newsRelease")) {
      ...
      if (Utils.isNotEmpty(newsRelease)) {
        Node newsJcrNode = WCMUtils.getNode(resourceResolver.getResource(newsRelease));
        if (newsJcrNode != null) {
          ListItem listItem = new ListItem(newsJcrNode.getPath(), currentPage, resource);
          newsRelease = listItem.getURL();

        } else {
          log.warn("**** Not a valid news release: " + newsRelease + " ****");
        }
      } else {
        log.info("**** News Release is empty ****");
      }
    }
...
<div id="metadata">
    <div class="metadata_left">
        <div id="md_main_photo"><img id="switcherTarget" alt="<fmt:message key="original"/>" /></div>
        <div class="clearfloat"></div>
        <input type="hidden" id="dphoto" value="<%=displayPhoto%>"></input>
        <div id="md_mPhoto">          
          <%if (Utils.isNotEmpty(itemPath)) {%>  
            <img src="/etc/designs/gnb/images/image.png">
            <a href="<%=itemPath%>" class="imageSwitcher" id="photo1" name="photo1">Photo 1</a>
            <input type="hidden" id="photo1link" value="<%=title%>"></input>
            <input type="hidden" id="photo1content" value="<%=content%>"></input>
            &nbsp;&nbsp;                        
              <a href="<%=itemPathBQ%>" id="bestphoto1" class="best" name="best1" style="display:none;" onclick="window.open('<%=itemPathBQ%>', 'popup'); return false">
                <fmt:message key="best"/>            
                <img src="/etc/designs/gnb/images/icon_external_link.png">
                &nbsp;
              </a>
          <%}if (photo2Prop != null) {%>    
            <c:out value=" | "/>
            ...
            <input type="hidden" id="photo2link" value="<%=photo2link%>"></input>
            <input type="hidden" id="photo2content" value="<%=photo2content%>"></input>
            &nbsp;&nbsp;            
              <a href="<%=path2BQ%>" id="bestphoto2" class="best" name="best2" style="display:none;" onclick="window.open('<%=path2BQ%>', 'popup'); return false">
                <fmt:message key="best"/>            
                <img src="/etc/designs/gnb/images/icon_external_link.png">
              &nbsp;
              </a>               
          <%}if (photo3Prop != null) {%>
            ...
            <input type="hidden" id="photo3link" value="<%=photo3link%>"></input>
            <input type="hidden" id="photo3content" value="<%=photo3content%>"></input>
            &nbsp;&nbsp;                                      
              <a href="<%=path3BQ%>" id="bestphoto3" class="best" name="best3" style="display:none;" onclick="window.open('<%=path3BQ%>', 'popup'); return false">
                <fmt:message key="best"/>            
                <img src="/etc/designs/gnb/images/icon_external_link.png">
              </a>
          <%
    ...
    <%if ( Utils.isNotEmpty(newsRelease) || hasVideo || hasAudio || Utils.isNotEmpty(presentation) || hasLink ) {%>
      <div class="related_box">      
        <div class="related_header"><h3><%=LocaleSupport.getLocalizedMessage(pageContext, "related")%></h3></div>                         
          ...
          <%if (Utils.isNotEmpty(newsRelease)) {%>
          <div id="md_newsRelease"><img src="/etc/designs/gnb/images/news.png">
            <a href="<%=newsRelease%>"><fmt:message key="newsRelease"/></a>
          </div>              
          <%}%>
          
          <%if (hasVideo) {%>              
            <div id="md_video"> <img src="/etc/designs/gnb/images/video.png">  
            <%if (videoOProp != null) {%>                             
          ...
          <%if (hasAudio) {%>
            <div id="md_audio"><img src="/etc/designs/gnb/images/sound.png">
            <%if (audioOProp != null) {%>                 
          ...
          <%if (Utils.isNotEmpty(presentation)) {%>
            <div id="md_presentation"><img src="/etc/designs/gnb/images/presentation.png">
