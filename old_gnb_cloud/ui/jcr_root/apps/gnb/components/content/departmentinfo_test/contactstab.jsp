...
try {
  ...
  String googleMapLink = String.format("http://maps.google.com/maps?f=q&source=s_q&hl=%s&q=%s@%s,%s&ie=UTF8&z=14&iwloc=r3", lang, mapPin, p_orgDataNode.getProperty(P_LATITUDE).getString(), p_orgDataNode.getProperty(P_LONGITUDE).getString());
  ...
  //if(isDepartment) {
    ...
    if (p_orgDataNode != null && p_contactDataPage != null) {
      ...
      if (!isDepartment ) {  //if not a department - see if there are any media contacts in this branch/unit 
       ...
       else { // if it is a dept - then we want the media contact header - even if they don't have one
...
<script type="text/javascript">
  ...
  {
if( window.gmapURL ) {
  $('<iframe width="270" height="230" scrolling="no" class="map_holder" marginwidth="0" marginheight="0" src="' + gmapURL + '"></iframe>').appendTo($('#map_wrapper'))
}
  });
</script>
