...
try {
  ...
  if(result!=null && result.getTotalMatches()>0) {
    ...
    //String url = p_slingRequest.getRequestURL().toString().replace("/_jcr_content/mainContent_par/departmentinfo.servicestab","") + "?service_start=%s&service_hits=%s#services";
	String url = p_slingRequest.getRequestURL().toString().replace("/jcr:content/mainContent_par/departmentinfo.servicestab","") + "?service_start=%s&service_hits=%s#services";      
    ...
    if(result.getResultPages().size()>1) {
      ...
      if(result.getPreviousPage()!=null) {
        out.write(String.format("<div class=\"item\"><a href=\""+  url + "\">%s</a></div>",
            result.getPreviousPage().getStart(),service_hits,LocaleSupport.getLocalizedMessage(pageContext, "previousText")));                  
      } 
      for(ResultPage resultPage : result.getResultPages()) {
        ...
        if(resultPage.isCurrentPage()) {
          ...
          out.write(String.format("<div class=\"item\"><a href=\""+  url + "\">%s</a></div>",resultPage.getStart(),service_hits,linkText));          
        }
      } // for each page
      if(result.getNextPage()!=null) {
        out.write(String.format("<div class=\"item\"><a href=\""+  url + "\">%s</a></div>",
            result.getNextPage().getStart(),service_hits,LocaleSupport.getLocalizedMessage(pageContext, "nextText")));                  
      }
      out.write("</div>");
    } // if 2+ pages
