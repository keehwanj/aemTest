...
while (rs.next()) 
    ...
    //creating node
    final Node gnbNode = WCMUtils.getNode(resourceResolver.getResource("/content/gnb/cnb_data/medialist/rs"));    
    nodeIter = gnbNode.getNodes();
    out.print(nodeIter.getSize());
    out.print("<br/>");    


    gnbNode.addNode("rs"+id, Nodes.PAGE.nodeType());//change 11 with id
    gnbNode.save();

    //setting jcr:content node
    final Node jcrContentNode = gnbNode.getNode("rs"+id);//change the "11" with id   
