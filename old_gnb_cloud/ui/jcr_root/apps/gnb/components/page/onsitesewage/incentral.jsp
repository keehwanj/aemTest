...
==============================================================================
  ...
  Onsite Sewage Incentral component
==============================================================================
--%>
...
String sDBQ = "C:\\database\\doh\\onsitesewage\\central.mdb";
...
while (rs.next()) 
    ...
    company_name = WordUtils.capitalizeFully(company_name);    //first letter capital
    out.println(company_name);
    out.println("\t");
  
    licence_no       =rs.getString("licence_no");
    last_name        =rs.getString("last_name");
    last_name        = WordUtils.capitalizeFully(last_name);    //first letter capital    
    first_name       =rs.getString("first_name");
    first_name       = WordUtils.capitalizeFully(first_name);    //first letter capital    
    
    city_name        =rs.getString("city_name");  
    city_name        =city_name.replace(", NB","");//replaceing ",NB"
    city_name        =city_name.replace(",NB","");//replaceing ",NB"
    city_name        =city_name.replace("NB",""); //replcing "NB"   
    ...
    //creating node
    final Node gnbNode = WCMUtils.getNode(resourceResolver.getResource("/content/gnb/doh-data/data/central"));    
    nodeIter = gnbNode.getNodes();
    out.print(nodeIter.getSize());
    out.print("<br/>");    


    gnbNode.addNode("c"+id, Nodes.PAGE.nodeType());//change 11 with id
    gnbNode.save();

    //setting jcr:content node
    final Node jcrContentNode = gnbNode.getNode("c"+id);//change the "11" with id   
    ...
    jcrNode.setProperty("en_non_conventional", type_license);
    jcrNode.setProperty("fr_non_conventional", type_license);

    jcrNode.setProperty("hideYesNo", "No");
