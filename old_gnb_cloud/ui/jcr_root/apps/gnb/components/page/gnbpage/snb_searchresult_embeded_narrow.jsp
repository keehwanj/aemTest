...
<tr>
  <td>
    <%if(contactPath.length()>0) { %><div><sling:include path="<%= contactPath + ".quicklink_snb.html" %>"/>&nbsp;(<%=titlename %>)</div><%} %>                               
    <%if(orgPath.length()>0) { %><div class="gnb_domain_added"><sling:include path="<%= orgPath + ".quicklink_snb.html" %>"/></div><%} %>      
...
<script>
	$(document).ready(function(){
        $('.gnb_domain_added').find('a').each(function(){ 
            var oldUrl = $(this).attr("href"); // Get current url
            if(!oldUrl.includes('http://www2.gnb.ca')){
            	var newUrl = oldUrl.replace("/content/gnb/en/contacts/", "https://www2.gnb.ca/content/gnb/en/contacts/").replace("/content/gnb/fr/contacts/", "https://www2.gnb.ca/content/gnb/fr/contacts/"); 
            }
