...
%>
<form action="<%=getLocalization("searchPath","/content/gnb/"+ lang + "/sitesearch",currentPage,log)%>.html" title="<%=getLocalization("searchHint","Search GNB.ca",currentPage,log) %>" method="get" id="keyword_search_form" onsubmit="if(this.q.value=='')return false;return true">
	<label for="q"><%=getLocalization("searchLabel","New Brunswick Site Search",currentPage,log)%></label>
    <input type="text" onclick="this.value='';" name="q" placeholder="<%=inputValue%>" class="searchText" id="q" />
	<%if(lang=="fr") { %>
...
$(function () {
  $("#q").autocomplete({
      source: function (request, response) {
        $.ajax({
          url: "http://clients1.google.com/complete/search?q=" + request.term + "&hl=<%=lang%>&client=partner&source=gcsc&partnerid=<%=partnerid%>&ds=cse&nocache=" + Math.random().toString(),
          dataType: "jsonp",
