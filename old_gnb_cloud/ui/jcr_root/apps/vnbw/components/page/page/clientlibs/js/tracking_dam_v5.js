...
jQuery(window).on('load', function(){
	...
	jQuery('a').each(function(){
    	...
    	if (badlinkJs.test(url)){
			...
			jQuery(this).attr("onclick","_gaq.push(['_trackEvent', 'Download', 'Jpg', '"+ url +"']);");
		}
    	else if (isDownloadAsx.test(url)){
			jQuery(this).attr("onclick","_gaq.push(['_trackEvent', 'Download', 'Asx', '"+ url +"']);");
		}
    	else if (isDownloadDoc.test(url)){
			jQuery(this).attr("onclick","_gaq.push(['_trackEvent', 'Download', 'Doc', '"+ url +"']);");
		}        
    	else { }
