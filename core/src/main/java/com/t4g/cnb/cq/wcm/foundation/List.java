      ...
      if (pageIterator == null) {
        try {
          ...
          if (SOURCE_QUERYBUILDER.equals(source)) {
            QueryBuilder queryBuilder = resource
              .getResourceResolver()
            ...
            if (session != null && queryBuilder != null) {
              try {
                Query query = queryBuilder.loadQuery(
                  resource.getPath() + "/" + SAVEDQUERY_PROPERTY_NAME,
                  session
                );
                if (query != null) {
                  ...
                  pageIterator =
                    new HitBasedPageIterator(result.getHits().iterator());
                  nodeIterator = result.getNodes();
                }
              } catch (Exception e) {
                log.error(
                  "error loading stored querybuilder query from " +
                  resource.getPath(),
                  e
                );
              }
            ...
            search.addPredicate(
              new Predicate("type", "type").set("type", NameConstants.NT_PAGE)
            );
            ...
            String[] statics = properties.get(
              PAGES_PROPERTY_NAME,
              new String[0]
            );
            Resource pr = request
              .getResourceResolver()
              .getResource(Text.getRelativeParent(resource.getPath(), 3));
